    ...
    fetch('cargarDatosTipo')
            ...
            .then(datos => {
				...
				<tr class="p-0" id="tr${element.id_tipo}">
					...
					<td>${estado}</td>
            		<td class="px-0 py-2">
						<button class="btnEditar text-center btn btn-warning" style="width:49%; margin:0px;" value="${element.id_tipo}" data-toggle="modal" data-target="#agregarTipo">EDITAR</button>
						<button class="btnEliminar text-center btn btn-danger" style="width:49%; margin:0px;"  value="${element.id_tipo}">ELIMINAR</button>
            		</td>
    			</tr>`
				});
